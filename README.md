...
`zig cc` automatically (MSVC is not supported by Go's cgo; see
`docs/adr/0006-sqlite-driver-mattn-cgo.md`).
...
remains our own code. The rationale, threat model, and review history are in
`docs/adr/0005-golang-jwt-verification.md`,
`docs/security/jwt-threat-model.md`, and `docs/security/reviews.md`.
...
```sh
gonew github.com/sandrorichi/webapp-template example.com/yourapp
```
...
5. This README and `CONTEXT.md`: replace template language with your app's
   domain language.
...
the SQLite amalgamation ships inside the pinned module — ADR-0006), in WAL
...
(ADR-0003): reversals are new up migrations, and the app refuses to start
