...
import (
	...
	"github.com/sr1ch1/webapp-template/web"
)
...
func StaticHandler() http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// mime.TypeByExtension reads the registry on Windows, so pin the
		// content types we serve to keep responses identical on every OS.
		// ServeContent only sniffs when Content-Type is unset.
